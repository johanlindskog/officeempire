...
export class MainScene extends Phaser.Scene {
  ...
  preload(): void {
    ...
    // Load furniture textures dynamically from registry
    for (const furniture of Object.values(FURNITURE)) {
      for (const [dir, path] of Object.entries(furniture.sprites)) {
        const key = `${furniture.id}_${dir}`;
        this.load.image(key, path);
      }
    }
  ...
  private updateTileSprite(x: number, y: number, cell: GridCell): void {
    ...
    if (cell.type === TileType.Road) {
      ...
      if (cell.buildingId) {
        ...
        const furniture = building ? null : getFurniture(cell.buildingId);
        const preservesTile =
          (building &&
            (building.category === "props" || building.isDecoration)) ||
          !!furniture; // Furniture also preserves tile
  ...
  private renderBuilding(
    ...
    const furniture = building ? null : getFurniture(buildingId);

    if (!building && !furniture) {
      console.warn(`Building/Furniture not found in registry: ${buildingId}`);
      return;
    }

    const key = `building_${originX},${originY}`;
    const textureKey = building
      ? this.getBuildingTextureKey(building, orientation)
      : this.getFurnitureTextureKey(furniture!, orientation);
    ...
    const footprint = building
      ? getBuildingFootprint(building, orientation)
      : getFurnitureFootprint(furniture!, orientation);

    // Get render size for slicing (use renderSize if available, else footprint)
    // Furniture doesn't have renderSize, so it always uses footprint
    ...
    // Check if we should slice (only for standard buildings)
    // Furniture sprites are small and don't support slicing logic used here
    const shouldSlice = !!building; // Only slice proper buildings

    if (!shouldSlice) {
      // Render simple sprite for furniture
      const simpleSprite = this.add.image(screenPos.x, bottomY, textureKey);
      simpleSprite.setOrigin(0.5, 1);
      // Simple depth: base on center of footprint
      const centerGridX = originX + footprint.width / 2;
      const centerGridY = originY + footprint.height / 2;
      const centerScreenY = GRID_OFFSET_Y + ((centerGridX + centerGridY) * TILE_HEIGHT) / 2;

      if (tint !== null) {
        simpleSprite.setTint(tint);
      }

      simpleSprite.setDepth(
        this.depthFromSortPoint(screenPos.x, centerScreenY, 0.05)
      );
      this.buildingSprites.set(key, simpleSprite);
      return;
    }
  ...
  private getFurnitureTextureKey(
    furniture: { id: string; sprites: Record<string, string> },
    orientation?: Direction
  ): string {
    const dirMap: Record<Direction, string> = {
      [Direction.Down]: "south",
    ...
    if (furniture.sprites[dir]) {
      return `${furniture.id}_${dir}`;
    }

    if (furniture.sprites.south) {
      return `${furniture.id}_south`;
    }

    const firstDir = Object.keys(furniture.sprites)[0];
    return `${furniture.id}_${firstDir}`;
  }
  ...
  private updatePreview(): void {
    ...
    if (this.selectedTool === ToolType.RoadNetwork) {
      ...
      const furniture = getFurniture(this.selectedBuildingId);
      const building = furniture ? null : getBuilding(this.selectedBuildingId);
      if (!furniture && !building) return;

      // Get footprint based on current orientation
      const footprint = furniture
        ? getFurnitureFootprint(furniture, this.buildingOrientation)
        : getBuildingFootprint(building!, this.buildingOrientation);
      ...
      // Only show lot tiles for non-decorative buildings and furniture (decorations preserve underlying tile)
      if (!isDecoration) {
        for (let dy = 0; dy < footprint.height; dy++) {
          for (let dx = 0; dx < footprint.width; dx++) {
            const tileX = originX + dx;
      ...
      // Always show building/furniture preview, but tint red if collision
      const textureKey = building
        ? this.getBuildingTextureKey(building, this.buildingOrientation)
        : this.getFurnitureTextureKey(furniture!, this.buildingOrientation);
      if (this.textures.exists(textureKey)) {
        const frontX = originX + footprint.width - 1;
        const frontY = originY + footprint.height - 1;
        const screenPos = this.gridToScreen(frontX, frontY);
